...
const MainContent = ({
  ...
  const studentMarks = subjectData
    ? subjectData.students.map((s, idx) => ({
        name: s.fullName,
        roll: idx + 1, // since roll no is not in your data, auto-generate
  ...
  return (
    <div className="flex-1 px-8 pt-8 space-y-8 overflow-x-auto h-full w-full">
        ...
        <>
          ...
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-semibold">
              {selectedDiv.exam} - Class {selectedDiv.class} Div {selectedDiv.div}
            </h2>

            {/* Progress + Subject Dropdown */}
            <div className="flex items-center gap-4">

              {/* Circular Progress for Subjects Marked */}
              <CircularProgress
                value={gradedCount}
                max={totalSubjects}
              ...
              {/* Published Status and Toggle Button */}
              <button
          ...
          <div className="flex gap-4">
            <div className="grid grid-cols-1 gap-4 w-[30%]">
              <div className="p-6 bg-white dark:bg-customDarkFg shadow rounded-xl text-center">
                <p className="text-sm text-gray-500 dark:text-gray-200">Class Average</p>
              ...
              <div className="p-6 bg-white dark:bg-customDarkFg shadow rounded-xl text-center">
                <p className="text-sm text-gray-500 dark:">Teacher</p>
                <p className="text-2xl font-bold text-gray-800 dark:text-white">
              ...
              <div className="p-6 bg-white dark:bg-customDarkFg shadow rounded-xl text-center">
                <p className="text-sm text-gray-500 dark:text-gray-200">Total Marks</p>
                <p className="text-2xl font-bold text-gray-800 dark:text-white">
      ...
      {showModal && (
        <Modal
          title="Subject Marking Status"
          onClose={() => setShowModal(false)}
        >
          <ul className="space-y-2">
            {allSubjects.map((subj) => {
              const graded = marksData.subjects.find(
                (s) => s.subject === subj
              );
              return (
                <li
